...
namespace i2c_pwm
{
  ...
  void Pca9685Impl::setFrequencyHz(uint16_t value)
  {
    ...
    frequencyScale_ = static_cast<float>(frequencyHz_) / static_cast<float>(DEFAULT_FREQ_HZ);

    //                  ( OSC_CLOCK_HZ )
    // prescale = round( -------------- ) - 1
    //                  (  4096 x rate )
    const uint8_t prescale = std::round(OSC_CLOCK_HZ / (static_cast<float>(MAX_VALUE) * static_cast<float>(clampedFreqModHz))) - 1;
